...
plt.errorbar(np.arange(len(ub_est)) + 1, ub_est, ub_err, color="#048c7f")
plt.xlabel(r"Number of blocks $b$", fontsize=15)
plt.ylabel("Bound estimation", fontsize=15)
plt.savefig("block_ub.pdf", bbox_inches="tight")

plt.figure(figsize=(5, 5 * 6/8))
plt.errorbar(np.arange(len(lb_est)) + 1, lb_est, lb_err, color="#048c7f")
plt.xlabel(r"Number of blocks $b$", fontsize=15)
plt.ylabel("Bound estimation", fontsize=15)
