...
def test_load_individual():
    ...
    assert (ca.K1.size == ca.ntransitions)
    assert np.isclose(ca.K1[0], 0, atol=0, rtol=1e-17)
...
def test_linfit():
    ...
    (betas_odr, sig_betas_odr, k1s_odr, ph1s_odr,
        sig_k1s_odr, sig_ph1s_odr, cov_k1_ph1s) = perform_odr(
        ca.nutil_in, ca.sig_nutil_in)

    (betas_linreg, sig_betas_linreg, k1s_linreg, ph1s_linreg,
        sig_k1s_linreg, sig_ph1s_linreg) = perform_linreg(ca.nutil_in)
    ...
    assert np.all(np.isclose(k1s_odr, k1s_linreg, atol=0, rtol=1e-2))
    assert np.all(np.isclose(ph1s_odr, ph1s_linreg, atol=0, rtol=1e-2))
    assert np.all(np.isclose(sig_k1s_odr, sig_k1s_linreg, atol=0, rtol=1))
...
def test_set_fit_params():
    ...
    kappa1temp = np.array(list(range(ca.ntransitions - 1)))
    ph1temp = np.array([np.pi / 2 - np.pi / (i + 2) for i in
        range(ca.ntransitions - 1)])
    alphaNPtemp = 1 / (4 * np.pi)
    thetatemp = np.concatenate((kappa1temp, ph1temp, alphaNPtemp),
        axis=None)
    ca._update_fit_params(thetatemp)
    assert (np.sum(ca.k1) == np.sum(kappa1temp))
    assert (np.sum(ca.K1) == np.sum(kappa1temp))
    assert (len(ca.K1) == (len(kappa1temp) + 1))
    ...
    theta_Mathematica = np.concatenate((kappa1nit_Mathematica,
        ph1nit_Mathematica, alphaNP_Mathematica), axis=None)
    ca._update_fit_params(theta_Mathematica)
    assert (np.sum(ca.K1) == np.sum(kappa1nit_Mathematica))
    assert (np.sum(ca.k1) == np.sum(kappa1nit_Mathematica))
    ...
    theta_LL_Mathematica = np.concatenate((kappa1nit_LL_Mathematica,
            ph1nit_LL_Mathematica, 0.), axis=None)
    ca._update_fit_params(theta_LL_Mathematica)
    assert (np.sum(ca.K1) == np.sum(kappa1nit_LL_Mathematica))
...
def test_constr_dvec():
    ...
    # with NP
    theta_LL_Mathematica_1 = np.concatenate((kappa1nit_LL_Mathematica,
