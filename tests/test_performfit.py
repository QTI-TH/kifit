...
from kifit.performfit import (
    ...
    sample_alphaNP_det,
    sample_alphaNP_fit,
    sample_alphaNP_fit_fixed_elemparams,
)
...
def test_linfit():
    ...
    (betas_odr, sig_betas_odr, kperp1s_odr, ph1s_odr, sig_kperp1s_odr, sig_ph1s_odr) = (
        perform_odr(
            ca.mu_norm_isotope_shifts_in,
            ca.sig_mu_norm_isotope_shifts_in,
            reftrans_index=0,
        )
    )
    ...
    (
        ...
        sig_kperp1s_linreg,
        sig_ph1s_linreg,
    ) = perform_linreg(ca.mu_norm_isotope_shifts_in, reftrans_index=0)
    ...
    betas_dat = np.array(
        [np.polyfit(xvals, yvals[:, i], 1) for i in range(yvals.shape[1])]
    )
...
def test_sample_alphaNP_det():
    ...
    alphas_GKP_mphivar, sigalphas_GKP_mphivar = sample_alphaNP_det(
        ca, 3, 100, mphivar=True
    )
    alphas_NMGKP_mphivar, sigalphas_NMGKP_mphivar = sample_alphaNP_det(
        ca, 3, 100, mphivar=True, gkp=False
    )
...
def test_sample_alphaNP_fit():
    ...
    assert np.abs(mean_alpha_elemvar - mean_alpha_elemfixed) < sig_alpha_elemfixed
    assert np.abs(mean_alpha_elemvar - mean_alpha_elemfixed) < sig_alpha_elemvar
